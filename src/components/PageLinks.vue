...
<template>
  <div class="pages" v-if="pageInfo.lastPage > 1">
    <button class="page-btn" :disabled="pageInfo.currentPage === 1" @click="go(1)">« {{ $t('gallery.pagination.first') }}</button>
    ...
    <button class="page-btn" :disabled="pageInfo.currentPage === pageInfo.lastPage" @click="go(pageInfo.lastPage)">{{ $t('gallery.pagination.last') }} »</button>
    ...
    <span class="pages-jump">
      ...
      / {{ pageInfo.lastPage }}
      <button class="page-btn" @click="go(jumpTo)">{{ $t('gallery.action.go') }}</button>
    </span>
  </div>
</template>
