...
import { items, countryLabel, timelines } from '../composables/useGalleryData.js'
import {
...
const summary = computed(() => [
  { label: t('gallery.section.collection'), value: filterSummary.value },
        ...
        <div id="timeline-link-box" v-if="showTimelineLink">
          ...
          <p>
            ...
            <RouterLink :to="{ name: 'timeline-results', query: { c: filters.country, start: filters.start, end: filters.end } }">
              {{ $t('gallery.section.timeline') }} | {{ countryLabel(countryIdForCode(filters.country)) }}
            </RouterLink>
          </p>
        </div>
      </aside>
