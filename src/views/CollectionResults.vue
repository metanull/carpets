...
import { items, countries, tags, countryLabel, timelines, countryById } from '../composables/useGalleryData.js'
import {
...
const filterSummary = computed(() => {
  ...
  if (query.value.start) parts.push(`${t('gallery.filter.from')} ${query.value.start}`)
  if (query.value.end) parts.push(`${t('gallery.filter.to')} ${query.value.end}`)
...
<template>
  <div id="collection-results-container">
    ...
    <div id="info-container">
      <p>{{ $t('gallery.section.collection') }} | <span>{{ filterSummary }}</span></p>
      <p>{{ page.total }} {{ $t('gallery.results.outOf') }} {{ total }} {{ $t('gallery.results.objects') }}</p>
    </div>
    ...
    <div id="content-container">
      ...
      <div id="no-results" v-else>
        {{ $t('gallery.results.noResults') }}
        <button class="linkish" @click="resetFilters()">{{ $t('gallery.action.resetFilters') }}</button>
      </div>

      <aside id="options-container">
        <div id="refine">
          <div class="options-label">{{ isFirstSearch ? $t('gallery.facet.filterBy') : $t('gallery.facet.filterFurtherBy') }}</div>
          ...
          <div id="reset-container">
            <button class="legacy-button" @click="resetFilters()">{{ $t('gallery.action.resetFilters') }}</button>
          </div>
        </div>

        <div id="timeline-link-box" v-if="showTimelineLink">
          <div class="options-label">{{ $t('gallery.results.timelineForSearch') }}</div>
          <p>
            ➤
            <RouterLink :to="{ name: 'timeline-results', query: { c: query.country, start: query.start, end: query.end } }">
              {{ $t('gallery.section.timeline') }} | {{ countryLabel(codeToId(query.country)) }}
            </RouterLink>
          </p>
        </div>
      </aside>
